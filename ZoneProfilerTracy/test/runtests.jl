...
# Build map of `@zone` names to line number:
function parse_zone_lines!(zone_lines::Dict{String,String}, file::AbstractString)
    lines = split(String(read(file)), "\n")
    for (idx, line) in enumerate(lines)
        match = findfirst("@zone profiler ", line)
        if !isnothing(match)
            name = split(line[match[end]+1:end], '"')[2]
            zone_lines[string(name)] = string(idx)
        end
    end
end

const zone_lines = Dict{String,String}()
parse_zone_lines!(zone_lines, run_zones_path)
parse_zone_lines!(zone_lines, test_pkg_path)

if !connect_tracy_capture && !connect_tracy_gui
    include(run_zones_path)
else
    @info "Spawn the headless tracy profiler, run the test script, and export the results to a CSV"
